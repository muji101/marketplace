        ...
        <div class="grid grid-cols-8 h-screen">
            <div class="col-span-2 pr-8">
                ...
                <div id="tabs" class="flex flex-col space-y-2">
                    ...
                    <a href="#{{ $id++ }}" id="default-tab" class="py-2 flex space-x-2 items-center bg-blue-100 rounded-lg p-4">
                        <img class="w-12 h-12 object-cover border-2 rounded-full" src="{{ asset('/storage/'.$chat->friend->photo) }}" alt="">
                        <div class="">
                            <p class="font-bold text-xs text-gray-600">{{ $chat->friend->name }} <span class="p-1 text-blue-400 rounded text-xs bg-blue-100">pembeli</span></p>
                            <p class=" text-gray-400 text-xs">{{ Str::limit($chat->subchat->where('user_id', '!=', Auth::user()->id)->last()->message, 20, '...') }}</p>
                        </div>
                    </a>
                    @endforeach
                </div>
                ...
                <div  id="{{ $num++ }}" class="flex-1 p-2 justify-between flex flex-col h-screen">
                    <div class="flex sm:items-center justify-between py-3 border-b-2 border-gray-200">
                        <div class="flex items-center space-x-4 px-4">
                            <img src="{{ asset('/storage/'.$message->friend->photo) }}" alt="" class="w-10 sm:w-16 h-10 sm:h-16 rounded-full">
                            <div class="flex flex-col leading-tight">
                                <div class="text-xl mt-1 flex items-center">
                                <span class="text-gray-700 mr-3 font-bold">{{ $message->friend->name }}</span>
                                <span class="py-1 px-2 text-blue-400 rounded text-xs bg-blue-100 font-bold">pembeli</span>
                                </div>
                                {{-- online offline status --}}
                                @if (Cache::has('user-is-online-' . $message->friend->id))
                                    <span class="text-xs text-green-400 font-bold">• Online</span>
                                @else
                                    <div class="text-xs text-green-400 font-bold">Online <span class="text-gray-400">{{ \Carbon\Carbon::parse($message->friend->last_seen)->diffForHumans() }}</span></div>
                                @endif
                            </div>
                        </div>
                    </div>
