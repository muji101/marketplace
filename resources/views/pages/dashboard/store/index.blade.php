...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <a href="{{ route('stores.create') }}" class="btn btn-primary shadow-md mr-2">Add New Store</a>
        <div class="dropdown">
                ...
                <tr class="intro-x">
                    <td class="w-40">
                        <div class="flex">
                            <div class="w-10 h-10 image-fit zoom-in">
                                <img data-action="zoom"  alt="photo" class="tooltip rounded-full" src="{{ asset('/storage/'.$store->photo) }}" title="Uploaded at {{ $store->created_at }}">
                            </div>
                        </div>
                    </td>
                    <td>
                        <a href="" class="font-bold whitespace-nowrap">{{ $store->name }}</a> 
                        <div class="text-gray-600 text-xs whitespace-nowrap mt-0.5">{{ $store->user->name }}</div>
                    </td>
                    <td>{{ $store->address }}</td>
                    <td class="text-center text-theme-9">Rp.1.515.123</td>
                    {{-- <td class="text-center text-theme-9">{{ $store->product->count() }}</td> --}}
                    <td class="text-center text-theme-9">{{ $store->type }}</td>
                    <td class="w-40">
                        @if ($store->status == 1)
                    ...
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3 text-theme-12" href="{{ route('stores.show', $store->id) }}"> <i data-feather="eye" class="w-4 h-4 mr-1"></i> Show </a>
                            <a class="flex items-center mr-3" href="{{ route('stores.edit', $store->id) }}"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
